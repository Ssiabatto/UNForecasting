...
def plot_forecast(historical_data, predictions, future_periods, column, output_dir):
    try:
        ...
        plt.plot(historical_data["PlotPeriod"], historical_data[column], marker='o', label='Historical Data')
        plt.plot(all_plot_periods[-(len(predictions) + 1):], all_values[-(len(predictions) + 1):], marker='o', linestyle='--', label='Forecast')
